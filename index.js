        ...
        try {
            ...
            if (this.firstUrl && u.host != this.firstUrl.host) {
                request.get(url, requestOpt(), function (error, response, body) {
                    if (!error && response.statusCode == 200 && response.headers['content-type'].search('image') != -1) tasks.waitQueue.push(url);
                });
                return;
            }
            ...
            request(url, requestOpt(), function (error, response, body) {
                if (!error && response.statusCode == 200) {
                    if (response.headers['content-type'].search('image') != -1) {
                        ...
                        try {
                            var strem = fs.createWriteStream(tasks.path + '/' + upath);
                            if (strem) request.get(url, requestOpt(), function (error, response, body) { }
                            ).pipe(strem);
                        } catch (e) {
